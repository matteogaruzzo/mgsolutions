...
export async function request(url, { method = 'GET', headers = {}, body, endpoint, ok = [] } = {}) {
  let lastError;
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      if (!TEMPORARY.has(response.status) || attempt === 2) throw lastError;
    } catch (error) {
      if (error instanceof HttpError && (!TEMPORARY.has(error.status) || attempt === 2)) throw error;
      lastError = error;
