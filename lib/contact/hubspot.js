...
// Nomi dei campi citati negli errori della Forms API (es. REQUIRED_FIELD):
// "Error in 'fields.nome'" o "Required field 'nome' is missing". Solo
// identificativi di proprietà (lettere, cifre, trattino basso).
function fieldNamesFromErrors(body) {
  const names = new Set();
  for (const error of body?.errors || []) {
    for (const match of String(error?.message || '').matchAll(/(?:fields\.|field\s+|')([a-z0-9_]{2,64})'/gi)) names.add(match[1]);
  }
  return [...names];
}
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        log('warn', 'form_submit_failed', {
          ...
          fields: fieldNamesFromErrors(error.body),
        });
        break;
      }
    }
