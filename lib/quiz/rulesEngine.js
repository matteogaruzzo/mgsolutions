...
export function buildRecommendation(responses) {
  ...
  // Piccola base per le automazioni: utile a prescindere dalla risposta
  // specifica, ma senza dominare la classifica se non emergono segnali più
  // forti dalle altre risposte.
  scores['automazioni-ai'] += 2;
  ...
  if (objective === 'vendere_online') {
    if (!hasEcommerceTool) {
      ...
      scores['automazioni-ai'] += 5; // gestire i nuovi contatti/ordini che arrivano
    } else {
      scores['automazioni-ai'] += 16; // wine club, abbonamenti, automazioni sullo store esistente
  ...
  if (objective === 'prenotazioni') {
    if (!hasBookingTool) {
      ...
      scores['automazioni-ai'] += 8;
    } else {
      scores['restyling-ottimizzazione'] += 7;
      scores['automazioni-ai'] += 6; // upselling e review automatiche sopra il booking che ha già
    }
  }
  ...
  // non costruire uno strumento da zero.
  if (objective === 'automatizzare') {
    scores['automazioni-ai'] += 10; // tracciamento preventivi, promemoria, follow-up automatici, turni
    if (hasCrmTool) {
      scores['automazioni-ai'] += 20;
    } else {
      scores['automazioni-ai'] += 7;
      if (!noTools) scores['automazioni-ai'] += 8;
    }
  }

  // Assistenza clienti e fidelizzazione.
  if (objective === 'assistenza_clienti') {
    scores['automazioni-ai'] += 17; // follow-up, storico conversazioni, segmentazione, richiesta recensione
    if (hasCrmTool) scores['automazioni-ai'] += 5;
  }
