...
export function buildRecommendation(responses) {
  ...
  // Piccola base per il software su misura: utile a prescindere dalla
  // risposta specifica, ma senza dominare la classifica se non emergono
  // segnali più forti dalle altre risposte.
  scores['software-ai-su-misura'] += 2;
  ...
  if (objective === 'vendere_online') {
    if (!hasEcommerceTool) {
      ...
      scores['software-ai-su-misura'] += 5; // gestire i nuovi contatti/ordini che arrivano
    } else {
      scores['ai-integration'] += 9; // wine club, abbonamenti, automazioni sullo store esistente
      scores['software-ai-su-misura'] += 7; // capire cosa rende davvero sullo store esistente
  ...
  if (objective === 'prenotazioni') {
    if (!hasBookingTool) {
      ...
      scores['software-ai-su-misura'] += 8;
    } else {
      scores['restyling-ottimizzazione'] += 7;
      scores['software-ai-su-misura'] += 6; // upselling e review automatiche sopra il booking che ha già
    }
  }
  ...
  if (objective === 'automatizzare') {
    scores['software-ai-su-misura'] += 10; // tracciamento preventivi, promemoria, follow-up automatici, turni
    if (hasCrmTool) {
      scores['ai-integration'] += 11;
      scores['software-ai-su-misura'] += 9;
    } else {
  ...
  if (objective === 'assistenza_clienti') {
    scores['software-ai-su-misura'] += 17; // follow-up, storico conversazioni, segmentazione, richiesta recensione
